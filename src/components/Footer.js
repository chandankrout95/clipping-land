...
export default function Footer() {
  ...
  return (
    <footer className="bg-bg-dark border-t border-white/5 pt-20 pb-10 mt-auto relative z-10">
      <div className="w-full max-w-[1280px] mx-auto px-6 grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr_1.5fr] gap-12 mb-16">
        {/* Brand */}
        <div className="flex flex-col gap-5">
          ...
          <p className="text-[0.95rem] text-text-secondary max-w-[280px]">
          ...
          <div className="flex gap-4 mt-2">
            {["Facebook", "Twitter", "Instagram"].map((label) => (
              <a
                key={label}
                href="#"
                className="w-[38px] h-[38px] rounded-full bg-white/[0.03] border border-white/[0.08] flex items-center justify-center text-text-secondary hover:bg-primary hover:text-white hover:border-primary hover:-translate-y-0.5 transition-all duration-300"
                aria-label={label}
              >
        ...
        <div>
          ...
          <ul className="flex flex-col gap-3.5">
            {[
              ...
              <li key={l.name}>
                <Link href={l.href} className="text-[0.95rem] text-text-secondary hover:text-white hover:pl-1 transition-all duration-300">
                  {l.name}
                </Link>
              </li>
            ))}
        ...
        {/* Legal Links */}
        <div>
          <h4 className="font-display text-white text-lg font-semibold mb-6">Legal</h4>
          <ul className="flex flex-col gap-3.5">
            {["Privacy Policy", "Terms of Service", "Refund Policy", "Contact"].map((name) => (
              <li key={name}>
                <Link href={name === "Contact" ? "/book-call" : "#"} className="text-[0.95rem] text-text-secondary hover:text-white hover:pl-1 transition-all duration-300">
                  {name}
                </Link>
              </li>
            ))}
        ...
        {/* Contact */}
        <div>
          <h4 className="font-display text-white text-lg font-semibold mb-6">Contact</h4>
          <ul className="flex flex-col gap-3.5">
            {[
              ...
              <li key={item.text} className="flex items-center gap-3 text-[0.95rem] text-text-secondary">
      ...
      {/* Bottom */}
      <div className="w-full max-w-[1280px] mx-auto px-6 border-t border-white/5 pt-8 flex flex-col md:flex-row items-center md:justify-between gap-4 text-center md:text-left">
        <p className="text-[0.85rem] text-text-muted">&copy; {currentYear} FlexNScale. All rights reserved.</p>
      </div>
    </footer>
  );
}
